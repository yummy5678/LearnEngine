...
int VulkanRenderer::init(GameWindow renderWindow)
{
	...
	try {
		...
		auto swapchainImage = m_SwapchainGenerator.GetImagesViews();
		...
		m_CommandGenerator.DrawFrame(renderPass, framebuffers[0], { {0,0},windowExtent }, graphicsPipeline);
		WriteVulkanImage(logicalDevice, physicalDevice,"../frame0.bmp", swapchainImage.GetImages()[2], windowExtent);
...
//		vkGetPhysicalDeviceSurfaceSupportKHR(device, i, surfaceGenerator.GetSurface(), &presentationSupport);
